...
class Logger {
  ...
  _readLevelFromUrl() {
    try {
      ...
      // accept multiple param names (fh_log, graaduation_log, debug)
      const candidate =
        params.get("fh_log") ?? params.get("gruduation_log") ?? params.get("debug");
  ...
  _readLevelFromLocalStorage() {
    try {
      if (!this.isBrowser) return null;
      const raw = window.localStorage?.getItem("GRADUATION_LOG_LEVEL");
      if (!raw) return null;
  ...
  setLevel(level, { persist = false, from = "manual" } = {}) {
    ...
    if (persist && this.isBrowser) {
      try {
        window.localStorage.setItem("GRADUATION_LOG_LEVEL", String(level));
      } catch {
        // ignore quota errors
      }
    }
    ...
    if (this.shouldLog) {
      ...
      const msg = `[GRADUATION] log level set -> ${label} (${parsed})${
        from ? ` via ${from}` : ""
      }`;
   ...
   * Messages should be { type: 'GRADUATION_LOG_LEVEL', level: 'warn' }
   */
  _enableRemoteControl() {
    ...
    window.addEventListener("message", (ev) => {
      try {
        ...
        const data = ev.data;
        if (!data || data.type !== "GRADUATION_LOG_LEVEL") return;
    ...
    window.addEventListener("storage", (ev) => {
      try {
        if (ev.key !== "GRADUATION_LOG_LEVEL") return;
