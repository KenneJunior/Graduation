...
document.addEventListener("DOMContentLoaded", () => {
  appLogger.time("Graduation app initialization");
  ...
  const state = {
    ...
    confetti: {
      number: 1, // Number of confetti pieces
      colors: [
        "#ff6b6b",
        "#ff8e53",
        "#ffd700",
  ...
  function init() {
    ...
    appLogger.info("Graduation application initialized successfully");
  ...
  async function playAudio() {
    ...
    try {
      audioLogger.debug("Attempting to play Graduation audio");
      await elements.GraduationAudio.play();
  ...
  function pauseAudio() {
    ...
    elements.GraduationAudio.pause();
    elements.playBtn.innerHTML =
  ...
  function triggerCelebration(delay = 0) {
    ...
    setTimeout(() => {
      ...
      if (!state.audio.isPlaying) {
        ...
        elements.GraduationAudio.currentTime = 0;
        playAudio();
      } else {
  ...
  init();
  appLogger.timeEnd("Graduation app initialization");
});
